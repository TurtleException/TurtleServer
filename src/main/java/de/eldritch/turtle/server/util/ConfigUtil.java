...
public class ConfigUtil {
     ...
     * @param relativePath Relative path to the configuration <b>not including</b> the <code>.yml</code> suffix. The
     *                     file will be located in the plugins' data folder.
     ...
     *                                       the default values.
     */
    public static YamlConfiguration getConfig(@NotNull String relativePath, @Nullable String resource) throws IOException, InvalidConfigurationException {
        File file = IOUtil.getFile(relativePath + ".yml");
    ...
    public static void applyDefaults(@NotNull FileConfiguration config, @NotNull String resource) throws IOException, InvalidConfigurationException {
        ...
        InputStream resourceStream = IOUtil.getResource(resource);
     ...
     */
    public static void validateVersion() throws InvalidConfigurationException {
        int version = Main.singleton.getConfig().getInt("configVersion", -1);
        int required;

        // retrieve required version
        try {
            YamlConfiguration defaultConfig = new YamlConfiguration();
            InputStream resourceStream = IOUtil.getResource("config.yml");
    ...
    private static void log(String... logMessages) {
        for (String logNote : logMessages) {
            TurtleServer.LOGGER.log(Level.INFO, logNote);
        }
    }
}
